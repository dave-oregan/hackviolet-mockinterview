...
class MockInterviewCore:
    def __init__(self, uuid, user_email, company_name, interview_level, interview_type):
    ...
    def _build_system_prompt(self, persona_prompt):
        return f"""
        {persona_prompt}
        Candidate: {self.personal_info['fullName']}, {self.personal_info['major']} at {self.personal_info['school']}.
    ...
    def generate_final_synthesis(self, audio_report, video_report):
        ...
        synthesis_prompt = f"""
    You are a Senior Executive Recruiter auditing a candidate's performance for {self.company_name}.
    
    ### INPUT DATA:
    AUDIO: {json.dumps(audio_report)}
    VIDEO: {json.dumps(video_report)}

    ### YOUR TASK:
    1. SYNTHESIZE: Combine findings into a cohesive narrative.
    2. SCORE: Provide a final_score (0-100).
